...
{
    /// <summary>
    /// Summary description for InvoicePassToUserNL.
    /// </summary>
    public class InvoicePassToUserNL : CBSolutions.ETH.Web.ETC.VSPage
    {
        #region WebControls
        ...
        private void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (Invoice.GetOverDueStatus(Convert.ToInt32(ViewState["InvoiceID"]), iTimeLimitInHours))
                {
        ...
        public void PopulateData()
        {
            ...
            RecordSet rs = null;
            lblRefernce.Text = "Invoice No.: " + rsInvoiceHead["InvoiceNo"].ToString();
            ...
            if (lblBuyer.Text.ToUpper().Trim().Equals("Akkeron"))
            {
            ...
            if (statusID != 1)
            {
                if ((statusID != 5 && statusID != 11 && statusID != 15 && statusID != 16) && (bExceedLimit == true))
                    bExceedLimit = false;
            }
            ...
            if (objInvoice.CompareUserIDForInvoice(Convert.ToInt32(Session["UserID"]), Convert.ToInt32(rsInvoiceHead["InvoiceID"])) || Convert.ToInt32(Session["UserTypeID"]) > 1)
                bCompareFlag = true;
            if (statusID == 3)
                statusID = 10;
            ...
            if (rsInvoiceHead["PassedToUserID"] != DBNull.Value && rsInvoiceHead["PreviousPassedToUserID"] != DBNull.Value)
            {
                if (Convert.ToInt32(rsInvoiceHead["PassedToUserID"]) == Convert.ToInt32(rsInvoiceHead["PreviousPassedToUserID"]))
                    iPassedToUserID = objInvoice.GetOldPassedToUserID(Convert.ToInt32(rsInvoiceHead["InvoiceID"]));
            }
            if (lblOverdue.Text == "")
            {
                ...
                {
                    rsUsers = da.ExecuteSP("up_GetPassingUsers", Convert.ToInt32(ViewState["InvoiceID"]), System.Convert.ToInt32(rsInvoiceHead["BuyerCompanyID"]), 0, "", 1);
                }
                else if (statusID == 3 || statusID == 10 || statusID == 4)
                {
                    ...
                    rsUsers = da.ExecuteSP("up_GetPassingUsers", Convert.ToInt32(ViewState["InvoiceID"]), System.Convert.ToInt32(rsInvoiceHead["BuyerCompanyID"]), iPassedToUserID, ApproverType, statusID);
                }
                else if (statusID == 8 || statusID == 2)
                {
                    try
                    {
                        ...
                        rsUsers = da.ExecuteSP("up_GetPassingUsers", Convert.ToInt32(ViewState["InvoiceID"]), System.Convert.ToInt32(rsInvoiceHead["BuyerCompanyID"]), iPassedToUserID, ApproverType, iStatID);
                    }
                    catch { }
                }
                ...
                {
                    rsUsers = da.ExecuteSP("up_GetPassingUsers", Convert.ToInt32(ViewState["InvoiceID"]), System.Convert.ToInt32(rsInvoiceHead["BuyerCompanyID"]), 0, ApproverType, statusID);
                }
                    ...
                    if (bExceedLimit)
                    {
                        ...
                        else
                        {
                            ...
                            if (rsInvoiceHead["ApprovedCEO"] != DBNull.Value)
                            {
                                ...
                                {
                                    rsUsers = da.ExecuteSP("up_GetPassingUsers", Convert.ToInt32(ViewState["InvoiceID"]), System.Convert.ToInt32(rsInvoiceHead["BuyerCompanyID"]), 0, "", statusID);
                                }
                                else
                                {
                                    if (statusID == 9)
                                    {
                                        ...
                                        rsUsers = da.ExecuteSP("up_GetPassingUsers", Convert.ToInt32(ViewState["InvoiceID"]), System.Convert.ToInt32(rsInvoiceHead["BuyerCompanyID"]), -1, "", statusID);
                                }
                            }
                            else
                            {
        ...
        {
            OutError.Visible = false;
            if (ddlDepartment.SelectedIndex == 0 && (cboStatus.SelectedValue.Equals("4") || cboStatus.SelectedValue.Equals("17")))
            {
                OutError.Text = "Please select a department.";
                OutError.Visible = true;
                return;
            }
            if (ddlNominalCode.SelectedIndex == 0 && (cboStatus.SelectedValue.Equals("4") || cboStatus.SelectedValue.Equals("17")))
            {
            ...
            rsInvoiceHead = Invoice.GetInvoiceHead(invoiceID);
            #region WHEN A USER IS APPROVING (1ST) AN INVOICE, IF THE USER IS NOT A MANAGEMENT USER. THEN THE USER CANNOT PASS THE INVOICE FOR 2ND APPROVAL TO HIMSELF. STARTS HERE.
            if (rsInvoiceHead["PassedToUserID"] != DBNull.Value)
            {
                ...
                {
                    OutError.Text = "Sorry, you are not authorised to pass an invoice to yourself for 2nd approval.";
                    OutError.Visible = true;
                    return;
                }
            }
            ...
            {
                if (Convert.ToInt32(cboStatus.SelectedValue) == 6 && txtComment.Text.Trim() != "" && ddlDepartment.SelectedIndex != 0 && ddlNominalCode.SelectedIndex != 0)
                {
                    if (objCompany.UpdateRejectStatus(Convert.ToInt32(rsInvoiceHead["InvoiceID"]), Convert.ToInt32(cboStatus.SelectedValue), "I"))
                    {
                        ShowMessage("Status updated successfully.");
                        hdWindowFlag.Value = "1";
                        objInvoice.UpdateCodes(Convert.ToInt32(ViewState["InvoiceID"]), Convert.ToInt32(ddlDepartment.SelectedValue), Convert.ToInt32(ddlProject.SelectedValue), Convert.ToInt32(ddlNominalCode.SelectedValue), txtDescription.Text.Trim());
                        return;
                    }
                }
                else
                {
                    ShowMessage("Please add comments and select department and nominal code for the current action.");
                    return;
                }
            }
            if (cboStatus.SelectedValue.Equals("17"))
            {
                if (cboStatus.SelectedValue.Equals("17") && Convert.ToInt32(Session["UserTypeID"]) > 1 && Convert.ToInt32(rsInvoiceHead["StatusID"]) == 6)
                {
                    if (objInvoice.UpdateInvoiceStatusAsDebited(Convert.ToInt32(rsInvoiceHead["InvoiceID"]), Convert.ToInt32(cboStatus.SelectedValue)))
                    {
                        ShowMessage("Status updated successfully.");
                        hdWindowFlag.Value = "1";
                        objInvoice.UpdateCodes(Convert.ToInt32(ViewState["InvoiceID"]), Convert.ToInt32(ddlDepartment.SelectedValue), Convert.ToInt32(ddlProject.SelectedValue), Convert.ToInt32(ddlNominalCode.SelectedValue), txtDescription.Text.Trim());
                ...
                {
                    ShowMessage("Sorry, cannot debit this invoice. It is not yet rejected.");
                    return;
                }
            }
            ...
            {
                OutError.Text = "Sorry, cannot pass invoice to a stanby at this stage.";
                OutError.Visible = true;
                return;
            }
            ...
            if (txtComment.Text.Equals(ViewState["oldComment"].ToString()) && (cboStatus.SelectedValue.Equals("2") || cboStatus.SelectedValue.Equals("6") || cboStatus.SelectedValue.Equals("8") || cboStatus.SelectedValue.Equals("13") || cboStatus.SelectedValue.Equals("14") || cboStatus.SelectedValue.Equals("15") || cboStatus.SelectedValue.Equals("16")
                ...
                || (cboStatus.SelectedValue.Equals("8") && Convert.ToInt32(rsInvoiceHead["StatusID"]) == 9)))
            {
            ...
            if (Page.IsValid)
            {
                ...
                strCurrentStatusID = objInvoice.GetCurrentStatus(Convert.ToInt32(ViewState["InvoiceID"]));
                ...
                {
                    OutError.Text = "Please select an user";
                    OutError.Visible = true;
                    return;
                }
                OutError.Visible = false;
                ...
                int Ret1 = da.ExecuteNonQuery("up_Invoice_updStatus", invoicePKID, Convert.ToInt32(Session["UserID"]) /*CBSAppUtils.AppUserId*/, stsId, rsInvoiceHead["PassedToUserID"], txtComment.Text, rsInvoiceHead["PassDate"], rsInvoiceHead["ApproveDate"], rsInvoiceHead["DepartmentID"], rsInvoiceHead["ProjectID"], rsInvoiceHead["NominalCodeID"],
                    ...
                    rsInvoiceHead["Approved2SB2"], rsInvoiceHead["ApprovedCEO"], txtDescription.Text.Trim(), Convert.ToInt32(ddlCodingDescription.SelectedValue));
                SendMailInfo();
                if (da.SPReturnValue == 2)
                {
                    Session.Add("DuplicateInvoice", "1");
                ...
                else
                {
                    ...
                    if (cboStatus.SelectedValue == "8" || cboStatus.SelectedValue == "2" || cboStatus.SelectedValue == "13" || cboStatus.SelectedValue == "14")
                    {
                        objInvoice.UpdatePreviousIDWhenUnderQuery(Convert.ToInt32(ViewState["InvoiceID"]), iPreviousStatusID, iPreviousPassedToUserID);
                    }
                    if (cboStatus.SelectedValue != "8" || cboStatus.SelectedValue != "2" || cboStatus.SelectedValue != "13" || cboStatus.SelectedValue != "14")
                    {
        ...
        private void LoadData()
        {
            ...
            for (int i = baseYear; i < (currentYear + 5); i++)
            {
        ...
        private void PopulateApproverDropDown()
        {
            ...
            dt = objInvoice.GetCodingDescriptionList();
        ...
        private bool CheckGross(RecordSet rsInvoiceHead)
        {
            ...
            DataAccess da = new DataAccess(CBSAppUtils.PrimaryConnectionString);
            RecordSet rsGrs = da.ExecuteSP("up_Invoice_Gross", Convert.ToInt32(rsInvoiceHead["InvoiceID"]));
            if (rsGrs != null)
            {
        ...
        private void ValidateDropDownOptions(int iStatusID)
        {
            ...
            else if (Convert.ToInt32(Session["UserTypeID"]) == 2 && iStatusID == 9)
            {
                PopulateStatusDropDown("2,8,6,7,9,17,18", 0, iStatusID);
            }
            else if (Convert.ToInt32(Session["UserTypeID"]) == 2 && iStatusID == 10)
            {
            ...
            else
            {
                ...
                {
                    if (iStatusID == 11 && Convert.ToInt32(Session["UserTypeID"]) == 2)
        ...
        private void PopulateStatusDropDown(string strList, int iDefaultValueToSelect, int iCurrentStatusID)
        {
            ...
            while (!rs1.EOF())
            {
                ...
                for (int counter = 0; counter < strListArray.Length; counter++)
                {
                    ...
                    {
                        iCounter++;
                        if (bExceedLimit == true && Convert.ToBoolean(rsInvoiceHead["ApprovedCEO"]) == false && iCurrentStatusID != 9)
                            PopulateStatusDropDown(11);
                    }
                }
                rs1.MoveNext();
        ...
        {
            string strApproverStatus = "";
            strApproverStatus = Invoice.GetApproverStatus(iInvoiceID);
        ...
        private void RemoveStatusBasedOnUserTypeID()
        {
            ...
            if (Convert.ToInt32(Session["UserTypeID"]) <= 1)
            {
                if (objInvoice.CheckApproverChainUser(Convert.ToInt32(ViewState["InvoiceID"]), Convert.ToInt32(Session["UserID"])) == false)
                {
                    cboStatus.ClearSelection();
            ...
            if (Convert.ToInt32(Session["UserTypeID"]) != 2)
            {
                ...
                if (Convert.ToInt32(Session["UserTypeID"]) == 2 || Convert.ToInt32(Session["UserTypeID"]) == 3 || Convert.ToInt32(Session["UserTypeID"]) == 1)
                {
                    try
                    {
                        cboStatus.Items.Remove(new ListItem("Delete/Archive", "7"));
                ...
                if ((Convert.ToInt32(Session["UserTypeID"]) == 4) && Convert.ToInt32(ViewState["StatusIDForHoldStatus"]) == 13 || Convert.ToInt32(ViewState["StatusIDForHoldStatus"]) == 14)
                {
                    try
                    {
                        cboStatus.Items.Remove(new ListItem("Delete/Archive", "7"));
        ...
        private bool CheckCreditNoteCreditedForInvoice(int iInvoiceID)
        {
            int iReturnValue = 0;
            bool bRetValue = true;
            string strMessage = "";
            iReturnValue = objInvoice.CheckCreditNoteCreditedForInvoice(iInvoiceID);
            if (iReturnValue == -101)
            {
                strMessage = "Sorry, cannot debit invoice because the credit note against this invoice has not yet been credited.";
                bRetValue = false;
            }
            else if (iReturnValue == -102)
            {
                strMessage = "Sorry, cannot debit invoice. No credit note exists against this invoice.";
                bRetValue = false;
            }
