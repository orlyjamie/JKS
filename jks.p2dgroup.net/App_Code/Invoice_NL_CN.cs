...
namespace JKS
{
    ...
    public class Invoice_NL_CN
    {
        ...
        #region GetCreditnoteDetails
        public DataTable GetCreditnoteDetails(int CreditnoteID)
        {
            ...
            {
                sqlConn.Open();
                SqlDataAdapter da = new SqlDataAdapter("select * from Creditnotedetail where CreditnoteId='" + CreditnoteID + "'", sqlConn);
                da.Fill(dt);
            }
        ...
        public int SaveInvoiceDatatoDataBase(int UserID, int InvoiceID, string strInvoiceNo, string strSupplierCompanyID, string strBuyerCompanyID,
            ...
            string strSupplierCountry, string strSupplierZIP, string strSellerVATNo, string strComments, string str22,
            decimal decNetTotal, decimal decVATAmt, decimal decTotalAmt, string strDeleteIDs, string strDepartmentID, string strBusinessUnitID)//Modified by Mainak 2018-03-19
        {
            ...
            try
            {
                sqlConn = new SqlConnection(CBSAppUtils.PrimaryConnectionString);
                sqlConn.Open();
                sqlCmd = new SqlCommand("usp_EditInvoiceTestBuyer_CN_New", sqlConn);//Modified by Mainak 2018-03-19
                sqlCmd.CommandType = CommandType.StoredProcedure;
                ...
                //Modified by Mainak 2018-03-19
                sqlCmd.Parameters.Add("@strDepartmentID", strDepartmentID);

                if (strBusinessUnitID == "")
                {
                    sqlCmd.Parameters.AddWithValue("@strBusinessUnitID", DBNull.Value);
                }
                else
                {
                    sqlCmd.Parameters.AddWithValue("@strBusinessUnitID", strBusinessUnitID);
                }
                //Modified by Mainak 2018-03-19
        ...
        //Added  by Mainak 2018-08-11
        public int checkInvoiceCrditNoteIDExist(int InvoiceID, string DocType)
        {
            int returnInvoiceID = 0;
            DataSet ds = new DataSet();
            sqlConn = new SqlConnection(CBSAppUtils.PrimaryConnectionString);
            sqlDA = new SqlDataAdapter("sp_InvoiceCrditNoteIDExist", sqlConn);
            sqlDA.SelectCommand.CommandType = CommandType.StoredProcedure;
            ...
            {
                returnInvoiceID = Convert.ToInt32(ds.Tables[0].Rows[0]["InvoiceId"]);
            }
        ...
        #region public void GetBatchTypes()
        public DataTable GetBatchTypes(int CompanyID)
        {
            DataSet ds = new DataSet();
            sqlConn = new SqlConnection(CBSAppUtils.PrimaryConnectionString);
            string strSql = "select BatchTypeID,BatchDocType from ScanBatchTypes where CompanyID= " + CompanyID;
        ...
        #region GetTaxRateByBuyerCompanyId
        public DataTable GetTaxRateByBuyerCompanyId(int buyerCompanyId)
        {

            DataSet Dst = new DataSet();
            SqlDataAdapter sqlDA = null;
            SqlConnection sqlConn = null;
            string sSql = "select TaxRate,CompanyID from TaxCodes where CompanyID=" + buyerCompanyId;
            sqlConn = new SqlConnection(CBSAppUtils.PrimaryConnectionString);
        ...
        //Added by Mainak 2018-09-06
        #region GetLineVatByBuyerCompanyId
        public DataTable GetLineVatByBuyerCompanyId(int buyerCompanyId)
        {
            DataSet Dst = new DataSet();
            SqlDataAdapter sqlDA = null;
            SqlConnection sqlConn = null;
            string sSql = "select * from company where CompanyID=" + buyerCompanyId;
            sqlConn = new SqlConnection(CBSAppUtils.PrimaryConnectionString);
            sqlDA = new SqlDataAdapter(sSql, sqlConn);
            try
            {
