...
namespace JKS
{
    ...
    public class Users : System.Web.UI.Page
    {
        ...
        //Added by kuntal karar on 1stJune2016 for #13
        ...
        private void SendEmail(string _from, string _to, string _cc, string _bcc, string _subject, string _body)
        {
            ...
            // Page.ClientScript.RegisterStartupScript(this.GetType(), "ErrorAlert", "alert(" + _SMTPServer.ToString() + ");", true);
            ErrorLog(Server.MapPath("ErrorLog.txt"), "SMTP:" + _SMTPServer.ToString() + " from " + _from.ToString() + " body " + _body.ToString() + " subject " + _subject.ToString());
        ...
        public void SendUserNamePassword(DataGrid dgGrid)
        {
            ...
            strMailFrom = ConfigurationManager.AppSettings["JKSLoginInfoMailFrom"].Trim();
            ...
            foreach (DataGridItem dgItem in dgGrid.Items)
            {
                ...
                if (chk.Checked)
                {
                    if (iCounter != Convert.ToInt32(dgItem.Cells[11].Text.Trim()))
                    {
                        ...
                        //Added by kuntal karar on 1stJune2016 for #13
                        int iReturnVal = 0;
                        string strPasswordnew = Guid.NewGuid().ToString().Substring(0, 8);
                        int strDbUserID = Convert.ToInt32(dgItem.Cells[11].Text.Trim());
                        iReturnVal = ForgotChangePassword(strDbUserID, objEncrypt.RijndaelEncription(strPasswordnew));
                        if (iReturnVal != -101)
                        {
                            //Addition ends by kuntal karar on 1stJune2016 for #13

                            strMailBody = strMailBody.Replace("$", "<BR>") + "URL = <A href= http://JKS.p2dgroup.net>http://JKS.p2dgroup.net</A><BR><BR>Kind regards,<BR><BR>Accounts Payable";

                            //blocked by kuntalkarar on 1stJune2016 for #13
                            /*strMailBody = strMailBody.Replace("@@UserName@@",strUserName).Replace("@@Password@@",strPassword);*/
                            //Added by kuntal karar on 1stJune2016 for #13
                            strMailBody = strMailBody.Replace("@@UserName@@", strUserName).Replace("@@Password@@", strPasswordnew);
                            ...
                            {
                                SendEmail(strMailFrom, ConfigurationManager.AppSettings["FeedBack"].Trim(), strCCMail, strBCCMail, strSubject + " :Error", strMailBody);
                            }
                            strMailBody = "";
                            ...
                            sqlCmd.Parameters.Add("@UserID", Convert.ToInt32(dgItem.Cells[11].Text.Trim()));
                            sqlConn.Open();
            ...
            try
            {
                while (!rs.EOF())
                {
                    ...
                    if (Convert.ToString(rs["EncryptPassword"]) != "")
                        ...
                        strPassWord = objEncrypt.RijndaelDecryption(Convert.ToString(rs["EncryptPassword"].ToString()));
        ...
        public bool CheckFirstLogin(int iUserID)
        {
            ...
            sqlConn = new SqlConnection(CBSAppUtils.PrimaryConnectionString);
            //sqlCmd = new SqlCommand("sp_CheckFirstLogin", sqlConn);//Commented By Rimi on 22nd July 2015
            sqlCmd = new SqlCommand("sp_CheckFirstLogin_GRH", sqlConn);// Addeded By Rimi on 22nd July 2015
            sqlCmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool SendUserNamePassword(string strUserName, string strPassword, string strEmail, string strMethod)
        {
            ...
            try
            {
                ...
                strMailBody = "<Font face='Verdana'>Welcome to the P2D Document Network.<BR><BR><BR>" +
                                ...
                                "please log onto the User Log-In section of www.p2dgroup.net<BR><BR>" +
        ...
        public DataSet GetUserType(int CompanyID)
        {
            ...
            try
            {
                //sqlDA = new SqlDataAdapter("Sp_UserTypeNew", sqlConn);//Commented By Rimi on 8th August 2015
                sqlDA = new SqlDataAdapter("Sp_UserTypeNew_ETC", sqlConn);
                sqlDA.SelectCommand.CommandType = CommandType.StoredProcedure;
        ...
        {
            sqlConn = new SqlConnection(CBSAppUtils.PrimaryConnectionString);
            //sqlDA = new SqlDataAdapter("sp_GetUserListCompanyWise_GRH", sqlConn);
            sqlDA = new SqlDataAdapter("sp_GetUserListCompanyWise_JKS", sqlConn);
        ...
        #region CheckIfUserInUse Added by Koushik Das as on 05-Apr-2017
        /// <summary>
        /// 
        /// </summary>
        /// <param name="iUserID"></param>
        /// <param name="iParentCompanyID"></param>
        /// <returns></returns>
        public int CheckIfUserInUse(int iUserID, int iParentCompanyID)
        {
            int iReturnValue = 0;
            ...
            {
                sqlConn = new SqlConnection(CBSAppUtils.PrimaryConnectionString);
                sqlCmd = new SqlCommand("sp_CheckIfUserInUse", sqlConn);
                sqlCmd.CommandType = CommandType.StoredProcedure;
                sqlCmd.Parameters.Add("@UserID", iUserID);
                sqlCmd.Parameters.Add("@ParentCompanyID", iParentCompanyID);
                sqlConn.Open();
                iReturnValue = Convert.ToInt32(sqlCmd.ExecuteScalar());
            }
            catch
            {
                iReturnValue = 0;
